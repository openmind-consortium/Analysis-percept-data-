...
for i=1:length(lfp_data.BrainSenseLfp)
    ...
    %time_start_s = datestr(time_start);
       %Extract Time points in ticks
        ...
        time_from_start=(LFP_data_ticks-LFP_data_ticks(1))/1000
        %Extract right Side
 ...
 subplot(2,1,1)
        ...
        %Plot LFP over time
        yyaxis left
        plot(time_from_start,right_LFP,time_from_start,right_stim)
        ylabel('LFP Power (unitless)')
        %Stimulation axis
       
        yyaxis right
        plot(time_from_start,right_stim)
        ...
        %Plot LFP over time
        yyaxis left
        plot(time_from_start,left_LFP,time_from_start,left_stim)
        ylabel('LFP Power (unitless)')
        
        %Stimulation axis
        yyaxis right
        plot(time_from_start,left_stim)
        ylabel('Stimulation (mA)')
        ylim([0 4])
